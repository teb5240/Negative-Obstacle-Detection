...
for i = 1:length(file_names)
    ...
    sec_pose = data_pose(:,2); % This does not begin at zero, so you will need to subtract the first value to get a time line that makes sence
%     t_sec = sec - sec(1);
    nsec_pose = data_pose(:,3)*10^(-9); % Again, this does not begin at zero
%     t_nsec = nsec_pose - nsec_pose(1);
%     t_pose = t_sec + t_nsec;
    t_pose = sec_pose + nsec_pose;
...
%     t_encoder_vel = t_sec_encoder_vel + t_nsec_encoder_vel;
    t_encoder_vel = sec_encoder_vel + nsec_encoder_vel;
    left_wheel_velocity = data_encoder_vel(:,4);
    right_wheel_velocity = data_encoder_vel(:,5);
    wheelchair_body_velocity = (left_wheel_velocity + right_wheel_velocity)/2;
    
    
    %%% Zero Time
    [~,min_time_ind] = min([t_pose(1) t_joy(1) t_encoder_vel(1)]);
    ...
    switch min_time_ind
        ...
        case 2
            min_time = t_joy(1);
        case 3
            min_time = t_encoder_vel(1);
    end
    
    t_pose = t_pose - min_time;
    t_joy = t_joy - min_time;
    t_encoder_vel = t_encoder_vel - min_time;
    
    %%% Function output
    pose_data{i} = [t_pose pose_x pose_y pose_psi];
